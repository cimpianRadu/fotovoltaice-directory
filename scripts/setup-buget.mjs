#!/usr/bin/env node
/**
 * Pregătește tabul „Leads" pentru bugetul orientativ (16 sept 2026):
 *   - lărgește grila la 49 de coloane, dacă e nevoie;
 *   - scrie antetul pentru AW („Buget").
 *
 ...
 *
 * Usage: node scripts/setup-buget.mjs
 */
...
const NEEDED_COLUMNS = 49; // până la AW inclusiv
...
// Plasa de siguranță: dacă pe AW1 stă altceva decât antetul așteptat și nu e
// gol, cineva a folosit deja coloana și a scrie peste ar rupe ce e dedesubt.
const current =
  (
    await sheets.spreadsheets.values.get({ spreadsheetId, range: 'Leads!AW1:AW1' })
  ).data.values?.[0] || [];
const occupied = current.filter((v, i) => v && v !== HEADER[i]);
if (occupied.length) {
  console.error(`STOP: AW1 conține deja „${occupied.join('", „')}". Nu scriu nimic.`);
  process.exit(1);
}
...
console.log('Leads: antet AW scris.');
