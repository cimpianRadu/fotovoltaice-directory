#!/usr/bin/env node
// One-shot: creates the "AlerteCVB" tab in the project Google Sheet with the
// header row that lib/sheets.ts:saveCvbAlertToSheet expects.
//
// Tabul ține înscrierile „anunță-mă când se deschide sesiunea Casa Verde
// Baterii" din ghidurile CVB. Capacitatea dorită și brandul de invertor sunt
// opționale, dar valoroase: pre-califică tehnic lead-ul înainte ca programul
// să existe (compatibilitatea invertor-baterie decide ce firme pot oferta).
//
// Usage: node scripts/create-alerte-cvb-sheet.mjs
...
const HEADER = [
  ...
  'invertor',
  'sursa',
  'status', // gol = de notificat; „Notificat <data>" după trimiterea alertei
];
