#!/usr/bin/env node
/**
 * Pregătește tabul „Leads" pentru intervalul preferat de apel (august 2026):
 *   - lărgește grila la 37 de coloane, dacă e nevoie;
 *   - scrie antetul pentru AK („Interval apel").
 *
 ...
 *
 * Usage: node scripts/setup-interval-apel.mjs
 */
...
// Plasa de siguranță: dacă pe AK1 stă altceva decât antetul așteptat și nu e
// gol, cineva a folosit deja coloana și a scrie peste ar rupe ce e dedesubt.
const current =
  (
    await sheets.spreadsheets.values.get({ spreadsheetId, range: 'Leads!AK1:AK1' })
  ).data.values?.[0] || [];
...
console.log('Leads: antet AK scris.');
